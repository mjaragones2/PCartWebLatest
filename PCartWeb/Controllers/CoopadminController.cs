...
namespace PCartWeb.Controllers
{
    ...
    public class CoopadminController : Controller
    {
        ...
        public ActionResult EditDriver(RegisterDriverViewModel model, HttpPostedFileBase file, HttpPostedFileBase file2)
        {
            ...
            if (getuser != null)
            {
                ...
                if (ValidateFile(model.ImageFile) == true)
                {
                    ...
                    location.Address = model.Address;
                    var logs = new UserLogs();
                    var user = db.Users.Where(x => x.Id == getuser.UserId).FirstOrDefault();
                    var userrole = user.Roles.Where(x => x.UserId == user.Id).FirstOrDefault();
                    var getuserrole = db2.Roles.Where(x => x.Id == userrole.RoleId).FirstOrDefault();
                    logs.Logs = model.Email + " account created";
                    logs.Role = getuserrole.Name;
        ...
        public ActionResult ActivateDriver(int? id)
        {
            ...
            if (id != null || id != 0)
            {
                var getdriver = db.DriverDetails.Where(x => x.Id == id).FirstOrDefault();
                var getuser = db.Users.Where(x => x.Id == getdriver.UserId).FirstOrDefault();
                if (getdriver != null)
                {
                    getdriver.IsActive = "true";
                    db.Entry(getdriver).State = EntityState.Modified;
                    db.SaveChanges();

                    var logs = new UserLogs();
                    var userrole = getuser.Roles.Where(x => x.UserId == getdriver.UserId).FirstOrDefault();
                    var getuserrole = db.Roles.Where(x => x.Id == userrole.RoleId).FirstOrDefault();
                    logs.Logs = getuser.Email + " account unlocked";
                    logs.Role = getuserrole.Name;
                    logs.Date = DateTime.Now.ToString();
                    db.Logs.Add(logs);
                    db.SaveChanges();
                    ...
                    #region formatter
                    string text2 = string.Format("Good Day our COOP Rider!", "Account Unlock Information", "We already unlocked your account.");
                    string html2 = "<br/><h4>You can now continue to serve customers and enjoy your ride from your COOP Shop. Thank you for patiently waiting!</h4>";
                    #endregion

                    MailMessage msg2 = new MailMessage();
                    msg2.From = new MailAddress(ConfigurationManager.AppSettings["AdminEmail"].ToString());
                    msg2.To.Add(new MailAddress(getuser.Email));
        ...
        public ActionResult DeactivateDriver(int? id)
        {
            var db = new ApplicationDbContext();
            if(id != null || id != 0)
            {
                ...
                if(getdriver!=null)
                {
                    ...
                    var userrole = getuser.Roles.Where(x => x.UserId == getdriver.UserId).FirstOrDefault();
                    var getuserrole = db.Roles.Where(x => x.Id == userrole.RoleId).FirstOrDefault();
                    logs.Logs = getuser.Email + " account locked";
                    logs.Role = getuserrole.Name;
                    logs.Date = DateTime.Now.ToString();
                    db.Logs.Add(logs);
                    db.SaveChanges();
                    ...
                    #region formatter
                    string text2 = string.Format("Good Day our COOP Rider!", "Account Lock Information", "We temporarily lock your account due to the severe violations.");
        ...
        public ActionResult ActivateMember(int? id)
        {
            ...
            if (id != null || id != 0)
            {
                var getmember = db.UserDetails.Where(x => x.Id == id).FirstOrDefault();
                var getuser = db.Users.Where(x => x.Id == getmember.AccountId).FirstOrDefault();
                if (getmember != null)
                {
                    getmember.MemberLock = "Active";
                    getmember.DateAccRetrieved = DateTime.Now.ToString();
                    var logs = new UserLogs();
                    var userrole = getuser.Roles.Where(x => x.UserId == getmember.AccountId).FirstOrDefault();
                    var getuserrole = db.Roles.Where(x => x.Id == userrole.RoleId).FirstOrDefault();
                    logs.Logs = getuser.Email + " account unlocked";
                    logs.Role = getuserrole.Name;
                    logs.Date = DateTime.Now.ToString();
                    db.Logs.Add(logs);
                    db.SaveChanges();
                    db.Entry(getmember).State = EntityState.Modified;
                    db.SaveChanges();


                    #region formatter
                    string text2 = string.Format("Good Day our COOP Member!", "Account Unlock Information", "We already unlocked your account.");
                    string html2 = "<br/><h4>You can now continue to shop and enjoy products from your COOP Shop. Thank you for patiently waiting!</h4>";
                    #endregion
        ...
        public ActionResult DeactivateMember(int? id)
        {
            ...
            if (id != null || id != 0)
            {
                ...
                if (getmember != null)
                {
                    getmember.MemberLock = "Locked";
                    getmember.DateAccLocked = DateTime.Now.ToString();
                    db.Entry(getmember).State = EntityState.Modified;
                    db.SaveChanges();
                    var logs = new UserLogs();
                    var userrole = getuser.Roles.Where(x => x.UserId == getmember.AccountId).FirstOrDefault();
                    ...
                    #region formatter
                    string text2 = string.Format("Good Day our COOP Member!", "Account Lock Information", "We temporarily lock your account due to the severe violations.");
                    string html2 = "<br/><h4>We will be notifying you through email once we have unlocked your account. Thank you, and have a great day ahead.</h4>";
                    #endregion
        ...
        public ActionResult DetailsOfDriver(int? id)
        {
            var db = new ApplicationDbContext();
            if(id == null || id == 0)
            {
            ...
            DriverDetailsModel viewdriver = db.DriverDetails.Where(x => x.Id == id).FirstOrDefault();
            if(viewdriver == null)
            {
                return HttpNotFound();
            }
            return View(viewdriver);
        }
        ...
        public ActionResult CoopSalesPDF()
        {
            ...
            try
            {
                ...
                String baseUrl = thisPageUrl.Substring(0, thisPageUrl.Length - "Coopadmin/CoopSalesPDF".Length);
        ...
        public ActionResult SalesOrderDetails(string id)
        {
            ...
            var orderdetails = db.ProdOrders.Where(x => x.UOrderId == id).ToList();
            ...
            decimal deliv = decimal.Parse(userorder.Delivery_fee.Value.ToString());
            viewDetail.TotalAmount = userorder.TotalPrice + deliv;
            ...
            if (payables != null)
            {
                foreach (var payable in payables)
                {
                    data.Add(new
                    {
                        ...
                        created_at = payable.Created_at.ToString(),
                        updated_at = payable.Updated_at.ToString(),
                    });
                }
            }
        ...
        public ActionResult ReturnRefundDetails(string id)
        {
            ...
            var orderdetail = db.UserOrders.Where(x => x.Id == returnRefunds.UserOrderId).FirstOrDefault();
            var returnRefundItems = db.ReturnRefundItems.Where(rr => rr.ReturnId.ToString() == id).ToList();
            var transactionrefund = db.TransactionReturns.Where(x => x.ReturnId == returnRefunds.Id).FirstOrDefault();
            ...
            {
                model.ConfirmedSenderName = transactionrefund.SenderName;
                model.ReceiptImage = transactionrefund.Receipt;
                model.ModeOfPayment = transactionrefund.ModeOfRP;
                model.ConfirmedSenderContact = transactionrefund.Contact;
                model.DateIssued = transactionrefund.Created_at.ToString();
            }

            model.ModeOfPayment = orderdetail.ModeOfPay;
            model.SenderName = coopDetails.Firstname + " " + coopDetails.Lastname;
            model.Contact = coopDetails.Contact;
        ...
        [HttpPost]
        public ActionResult AcceptReturn(ReturnRefundDetails model, HttpPostedFileBase file)
        {
            try
            {
                var db = new ApplicationDbContext();
                ...
                string id = Request["returnrefundId"];
                ...
                else if (getuserorder.ModeOfPay == "Cash On Delivery")
                {
                    string select = Request["selecteu"];
                    string contact = Request["senderContact"];
                    string sendername = Request["sendername"];
                    if (model.ImageFile == null || String.IsNullOrEmpty(select))
                    {
                        return RedirectToAction("ReturnRefundDetails", new { id = id });
                    }
                    var getuser = db.UserDetails.Where(x => x.AccountId == getuserorder.UserId).FirstOrDefault();
                    int orderid = int.Parse(id);
                    var allowedextension = new[] { ".jpeg", "jpg", ".png", ".gif", ".avif", ".svg" };
                    var name = Path.GetFileNameWithoutExtension(model.ImageFile.FileName);
                    var extension = Path.GetExtension(model.ImageFile.FileName);
                    if (allowedextension.Contains(extension.ToLower()))
                    {
                        var path = Path.Combine(Server.MapPath("~/TransactionRefundReceipts/"), name + extension);
                        model.ImageFile.SaveAs(path);
                        var transactionrefund = new TransactionReturnRefund { SenderName = sendername, Contact = contact, ModeOfRP = select, ReturnId = orderid, Created_at = DateTime.Now, Receipt = name + extension, Updated_at = DateTime.Now };
                        db.TransactionReturns.Add(transactionrefund);
                        db.SaveChanges();
                    }
                }
